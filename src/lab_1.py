...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--old_color", type=int, nargs=3, help="Old color to replace (RGB format)."
    )
    parser.add_argument(
        "--new_color", type=int, nargs=3, help="New color to apply (RGB format)."
    )

    parser.add_argument(
        "--red_balance", type=float, default=1.0, help="Red color balance multiplier."
    )
    parser.add_argument(
        "--green_balance",
        type=float,
        default=1.0,
        help="Green color balance multiplier.",
    )
    parser.add_argument(
        "--blue_balance", type=float, default=1.0, help="Blue color balance multiplier."
    )
    ...
    parser.add_argument(
        ...
        default="JPEG",
        help="Output image format (choices: 'PNG', 'JPEG', 'BMP', 'TIFF').",
    )
